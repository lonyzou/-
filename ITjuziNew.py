...
class juzi():
    def __init__(self):
        self.url = "http://radar.itjuzi.com/investevent/info?location=in&orderby=def&page={}"
        self.headers = {
    ...
    def get_url_list(self):
        url_list = [self.url.format(i) for i in range(1, 1935)]
        for url in url_list:
            self._parse_url(url)
